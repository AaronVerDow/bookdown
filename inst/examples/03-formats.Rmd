...
```{sh, echo=FALSE}
## running in R causes issues

combo_image() {
    input_pdf="$1"
    output_image="${2:-combined_pages.png}"

    [ -f "$output_image" ] && return

    temp_dir="./_bookdown_files/_cache/$( basename "$input_pdf" )"
    mkdir -p "$temp_dir"

    for page in {0..4}; do
        convert -density 150 "$input_pdf[$page]" -background white -alpha remove "$temp_dir/page_$page.png" && continue 

        # for any missing pages, copy page 0 and make it transparent
        # this keeps the image sizes identical 
        if [ ! -f "$temp_dir/blank.png" ]; then
            # white page
            # convert "$temp_dir/page_0.png" -alpha off -fill white -colorize 100% "$temp_dir/blank.png"
           
            # transparent page
            convert "$temp_dir/page_0.png" -fill none -draw "color 0,0 reset" "$temp_dir/blank.png"
        fi

        cp "$temp_dir/blank.png" "$temp_dir/page_$page.png"
    done

    montage "$temp_dir"/page_*.png -background "#EEEEEE" -geometry +10+0 -tile x1 -resize x400 "$output_image"
}

render() {
  output_dir="_book/pdf_examples"
  pdf="$output_dir/$1.pdf"
  if [ ! -f "$pdf" ]; then
    Rscript -e 'library(bookdown); render_book(input="./pdf_examples/'$1'", output_dir="../../'$output_dir'")'
    mv "$output_dir/_main.pdf" "$pdf"
  fi
  combo_image "$output_dir/$1.pdf" "$output_dir/$1.png"
}
...
Document class sets the layout standard. This includes whether the document is two sided, where the page numbers go, and the placement of title and table of contents. Two sided documenets are designed to be printed and bound into a book.  Page numbers are on the edge of the page, the margins vary between left and right, and blank pages are included so that new chapters always start on the right page.

The section below lists the common document_classes with some sample renderings of a book.

More documentation on these classes can be found here:
* https://en.wikibooks.org/wiki/LaTeX/Document_Structure#Document_classes
* https://www.ctan.org/pkg/koma-script

**book**

[![](./_book/pdf_examples/book.png)](./_book/pdf_examples/book.pdf)

**report**

[![](./_book/pdf_examples/report.png)](./_book/pdf_examples/report.pdf)

**article**

[![](./_book/pdf_examples/article.png)](./_book/pdf_examples/article.pdf)

**memoir**

[![](./_book/pdf_examples/memoir.png)](./_book/pdf_examples/memoir.pdf)

Class with more customization options than report and
 https://ctan.math.washington.edu/tex-archive/macros/latex/contrib/memoir/memman.pdf

**scrbook**

[![](./_book/pdf_examples/scrbook.png)](./_book/pdf_examples/scrbook.pdf)

**scrreport**

[![](./_book/pdf_examples/scrreport.png)](./_book/pdf_examples/scrreport.pdf)

### Configuration Examples

Below are some configuration examples, specifically for using building PDFs that are not complete books.
...
** Center text (do not alternate sides for a book) **
...
output: 
  bookdown::pdf_book:
    pandoc_args: --variable=geometry:a5paper
---
```
